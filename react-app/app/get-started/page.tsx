 ...
 *   1 now asks only property address + a one-tap "what do you need help
 *   with" chip (2 fields) before any account field is shown; Step 2 is the
 *   account form this file already had (name/email/password, Google or
 *   password). Phone is no longer required anywhere on this page — it never
 *   reached the `leads` insert or supabase.auth.signUp(), only the non-fatal
 *   cs_signup payload, so it did not need to gate account creation. No
 *   Supabase schema or Edge Function changed; SMS-consent and referrer
 *   opt-out checkbox copy is untouched (Tier C boundary). Closes on a
 *   fresh auth.users count 14 days post-deploy beside the pre-change
 *   baseline (2 signups / 7 days, 0 / 24h at 2026-09-08).
 *
...
export default function GetStartedPage() {
  ...
  /**
   * gh-1901: Step 1 — about the home, asked before any account field exists on
   * screen. Phone is deliberately not required here (or anywhere on this page
   * any more): it never reached the `leads` insert or supabase.auth.signUp()
   * to begin with (see persistSignupContext below), only the non-fatal
   * cs_signup localStorage payload, so gating account creation on it bought
   * nothing the backend needed.
   */
  const validateHomeInfo = (): string | null => {
    if (!address.trim() || !projectType) {
      return 'Please enter your property address and let us know what you need help with.';
    }
    return null;
  };
  ...
  const handleGoogle = async () => {
    ...
    // gh-1901: Google button now lives in Step 2, so address + projectType
    // are already guaranteed by the Step 1 → Step 2 transition below; only
    // name remains to check here.
              ...
              */
              ...
              <>
                ...
                {/*
                  ...
                  name fields below are still collected first — validateAccountProfile()
                  runs on click and persistSignupContext() stashes the payload
                  (including Step 1's address + projectType) before the browser
                  leaves for Google. gh-1901: this button now sits in Step 2 —
                  Step 1 (property address + project type) already ran first.
                */}
                <button
                  type="button"
                ...
                {/* Phone — gh-1901: no longer required. It never reached the
                    `leads` insert or supabase.auth.signUp() to begin with
                    (see persistSignupContext), only the non-fatal cs_signup
                    payload, so it did not need to gate account creation. */}
                <div className="form-group">
