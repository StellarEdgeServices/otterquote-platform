 ...
 * Reads the Supabase native auth token from sb-yeszghaspzwwstvsrioa-auth-token cookie,
...
export default async (req: Request, context: any) => {
  ...
  // Parse Supabase auth token from native cookie
  const cookieHeader = req.headers.get('cookie') || '';
  const match = cookieHeader.match(/(?:^|;\s*)sb-yeszghaspzwwstvsrioa-auth-token=([^;]+)/);
  const token = match?.[1];
