...
-- Reverts: gh916_progressive_partner_status_triggers.sql
-- Status: DRAFT ONLY — forward migration not yet applied (D-182 approval pending).
--
-- Restores claims_advance_referral() and apply_referral_commission() to their
-- current live bodies (byte-identical to baseline / gh-752 respectively), and
-- drops the new trigger + function added for the stage-2 (bid_received) signal.
-- No data is destroyed by this rollback — it only reverts function/trigger
-- definitions, not any rows written by send-partner-status-email itself
-- (referrals.metadata.partner_status_series stamps are left in place; that is
-- send-partner-status-email's own idempotency data, not owned by this migration).

BEGIN;

-- Drop the new stage-2 trigger + function entirely.
DROP TRIGGER IF EXISTS trg_notify_partner_status_on_bid_submitted ON public.quotes;
DROP FUNCTION IF EXISTS public.notify_partner_status_on_bid_submitted();

-- Restore claims_advance_referral() to its pre-migration body.
...
-- Restore apply_referral_commission() to its gh-752 (current live) body.
