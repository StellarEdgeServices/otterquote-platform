-- gh-1425 path 1: add has_payment_method, stripe_payment_method_id,
-- stripe_payment_method_last4 to both privileged-column freeze functions on
-- public.contractors. Ruled tier:3a by CTO cto-2026-09-01T02:16:49Z (extends an
-- existing guard; removes attacker capability, grants none). Forward SQL is the
-- R-097 notice diff (gh-1425, 2026-08-31T20:43:56Z) applied to the byte-exact
-- live bodies, comments preserved. Rollback file restores those live bodies.
-- verify-payment-method (service_role) is exempt from both functions and unaffected.
...
BEGIN
  ...
  IF NEW.status                  IS DISTINCT FROM OLD.status
     ...
     OR NEW.stripe_payment_method_last4 IS DISTINCT FROM OLD.stripe_payment_method_last4 THEN
