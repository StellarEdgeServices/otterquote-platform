-- Rollback for gh-1081: restore SECURITY DEFINER default on cert_verification views
--
-- Reverting WITH (security_invoker = true) back to the default SECURITY DEFINER
-- behavior, and restoring the broad default-privilege grants that existed
-- before the gh-1081 migration.
...
GRANT SELECT ON public.cert_verification_quality TO anon;
GRANT SELECT ON public.cert_verification_quality TO authenticated;
GRANT ALL ON public.cert_verification_quality TO postgres;
...
GRANT SELECT ON public.cert_verification_quality_by_manufacturer TO anon;
GRANT SELECT ON public.cert_verification_quality_by_manufacturer TO authenticated;
GRANT ALL ON public.cert_verification_quality_by_manufacturer TO postgres;
