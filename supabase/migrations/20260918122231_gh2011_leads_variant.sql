...
-- included here, and must not be added later.
--
...
--
-- Pre-flight, live against prod yeszghaspzwwstvsrioa (2026-09-18, pre-apply):
--   variant_col_exists=0, variant_check_exists=0, leads_total=59,
--   leads_router=1, role_check_exists=1 -- column and constraint both
--   absent, 59 rows total, leads_role_check present as the pattern
--   this file's ADD COLUMN block mirrors.
--
-- APPLIED TO PRODUCTION 2026-09-18T12:22:31Z by CEO claim
-- ceo-2026-09-18T11:12:34Z, under R-064's Gate 8 carve-out (tier:3a applied
-- by an executive on #2014; purely additive; rollback half in the same PR;
-- pre-flight pasted on the issue before execution). Applied BEFORE this PR
-- merged, deliberately: start.html deploys automatically from the repo while
-- this migration is applied by hand, so had the HTML shipped first, every
-- insertFreshLead() would have posted `variant` to PostgREST against a table
-- lacking the column -> PGRST204 -> `throw res.error` -> the lead destroyed.
-- On public.leads, schema leads and code follows -- never the reverse.
--
-- Post-apply verification, live: variant_col_exists=1, type text,
-- is_nullable YES, column_default null, any_variant_constraint=0,
-- leads_total=59, rows_with_variant=0.
--
-- This file's version matches the applied ledger entry (20260918122231
-- gh2011_leads_variant). Re-running it is a no-op.
