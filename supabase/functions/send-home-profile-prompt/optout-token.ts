...
//
// gh-2013: this copy is used by send-home-profile-prompt, a second Tier B
// promotional nudge template (D-231, post-completion home-profile prompt).
// It shares homeowner-email-optout's endpoint and OPTOUT_EVENT_TYPE with
// send-homeowner-next-steps ON PURPOSE — one signed token, one activity_log
// event type, one unauthenticated endpoint, reused rather than duplicated
// with a new event type. The practical effect: a homeowner who clicks
// "Stop these updates" on EITHER template's email stops BOTH. That is the
// intended scope of a single opt-out click, not an accident of reuse.
//
...
// `dashboard.html?profile_prompt=1` link, so the token adds no identifier the
// message did not already carry.
//
// ─── WHY THIS FILE IS DUPLICATED ────────────────────────────────────────────
// Identical copies live at supabase/functions/homeowner-email-optout/
// optout-token.ts and supabase/functions/send-homeowner-next-steps/
// optout-token.ts. This repo's Edge Function deploy path does not resolve
// `_shared/` imports (see send-home-profile-prompt/index.ts's own header
// comment and notify-contractors/index.ts line 91, which inline shared email
// code for the same reason), and no function in this repo imports across
// function directories. Duplication is the deployable shape. The other two
// copies are held byte-identical from the BEGIN marker down by
// homeowner-email-optout/optout-token.parity.test.ts; THIS copy is byte-
// identical to both as of this PR but is not yet wired into that parity
// test (noted in the PR description as follow-up scope, not silently
// assumed).
//
