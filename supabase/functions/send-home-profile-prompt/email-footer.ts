 ...
 *
 * gh-2013: `send-home-profile-prompt` is a THIRD Tier B promotional nudge
 * template (post-completion "build your home profile" prompt, D-231) — same
 * class of email as `send-homeowner-next-steps`, so the same requirement
 * applies. This copy is byte-identical to
 * `send-homeowner-next-steps/email-footer.ts`, carrying the SAME resolved
 * D-237 mailbox address — not a fresh Tier C legal decision, the same string
 * already ruled on for #1824 and reused for #1944.
 *
 ...
 * `./index.ts`) and is unrelated to the postal-address requirement. Both
 * lines appear in the footer; neither one satisfies the other.
 */

/** The D-237 mailbox address, same resolved value as #1824 / #1944. */
