...
// invite reminder email's unsubscribe link, "the same pattern as P-4" --
// this is a FOURTH copy of send-partner-onboarding/optout.ts's exact
// mechanism (already duplicated at partner-email-optout/optout-token.ts
// and meta-leadgen-webhook/optout.ts, both held byte-exact by their own
// parity tests). Deliberately the SAME secret
// (PARTNER_ONBOARDING_OPTOUT_SECRET) and the SAME endpoint
// (partner-email-optout), not a new invite-specific one: a partner who
// clicks "unsubscribe" on the initial invite email or this reminder is
// asking to stop partner marketing mail from Otter Quotes, not to opt out
// of one specific campaign -- the broader, more conservative reading is
// also the correct CAN-SPAM one. Reusing the existing, already-deployed
// secret and endpoint means this needs no new secret provisioning beyond
// what P-4 already has live.
//
...
// copy -- same convention that pair's own parity test already uses.
