...
// zero-row match as an error. On this function's money/dunning paths
// (payment_status flips, claim status, dunning-state transitions) that means
// the caller believes the write succeeded while nothing was actually
// written, with no error anywhere to notice it by.
//
...
// repo's existing per-function pure-logic convention (see
// docusign-webhook/price-verify.ts, docusign-webhook/ack-verify.ts, and this
// file's own live-charge-guard.ts) -- and NOT an import from
 ...
 * on a money/dunning path, so every site in this function uses the same
 * shape in `console.error` / `platform_alerts_log` output (op is the DB
 * write's short description, e.g. "quotes.payment_status=dunning for quote
 * q1").
 */
