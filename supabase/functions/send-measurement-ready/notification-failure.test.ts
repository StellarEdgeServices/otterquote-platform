...
// a failed Mailgun send inside send-measurement-ready bubbled to the
// generic outer catch and returned "Internal server error" with no
// activity_log/Sentry/notifications row anywhere (the fulfilment log row
// #1412 added upstream of this is a separate, already-durable write — this
// covers the homeowner "ready" email send itself).
// Run: deno test supabase/functions/send-measurement-ready/notification-failure.test.ts

import { assertEquals, assertExists } from "https://deno.land/std@0.177.0/testing/asserts.ts";
import { buildNotificationFailureRow, logNotificationFailure } from "./notification-failure.ts";

const CTX = {
  functionName: "send-measurement-ready",
...
Deno.test("buildNotificationFailureRow: fixed event_type/title, error/recipient_role/function in metadata", () => {
  ...
  assertEquals(row.metadata.function, "send-measurement-ready");
