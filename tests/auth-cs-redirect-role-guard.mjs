 ...
 * gates.
 *
 * REVIEW: FAIL rework (second re-review, current head). CONTRACTOR_GATED_FILES
 * was matched by exact '.html' filename, but Netlify Pretty URLs serve every one
 * of these pages extensionless (`/contractor-dashboard` -> 200,
 * `/contractor-dashboard/` -> 301 to it), and requireAuth() stamps whatever
 * window.location.pathname actually was — which under Pretty URLs is the
 * extensionless form. An exact-match comparison against '....html' missed that
 * form entirely, so the role-guard never ran and the homeowner was still
 * replayed to the contractor surface via a URL-form mismatch instead of a
 * role-precedence one. Cases 8-10 below pin the extensionless and
 * trailing-slash forms so this cannot regress silently.
 *
...
const CASES = [
  ...
  [
    'REVIEW: FAIL rework — Netlify Pretty URL extensionless form is still caught (no .html suffix)',
    { pathname: '/login.html', user: USER, profileRole: 'homeowner' },
    '/contractor-dashboard',
    '/trade-selector.html',
  ],
  [
    'REVIEW: FAIL rework — Netlify Pretty URL trailing-slash form is still caught',
    { pathname: '/login.html', user: USER, profileRole: 'homeowner' },
    '/contractor-dashboard/',
    '/trade-selector.html',
  ],
  [
    'REVIEW: FAIL rework — extensionless form still replays for a real contractor (no regression)',
    { pathname: '/login.html', user: USER, contractorRow: { id: 'c1' } },
    '/contractor-dashboard',
    '/contractor-dashboard',
  ],
