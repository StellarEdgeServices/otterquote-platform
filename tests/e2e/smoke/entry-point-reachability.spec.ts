 ...
 * uses (`OQ_BID_FORM_ACTIONS` / `onBidFormDelegatedClick`, bound by
 * `bindBidFormActions()` on both dynamic containers in
 * contractor-bid-form.html -- Part 2 landed these as the two separate
 * `onGutterGuardEntriesClick` / `onWarrantyCardsContainerClick` listeners
 * and #1867's merge resolution collapsed them into one, because both set the
 * same `el.dataset.oqActionsBound` flag and so could not coexist), so
 * `tools/inline_handler_attr_check.py` is green
